...
object AudioConverter {
    ...
    ): Result<File> = convertToMp3Internal(
        input = input,
        outputMp3 = outputMp3,
        metadata = metadata,
    ...
    private fun convertToMp3Internal(
        ...
        fun buildCmd(withCover: Boolean): String = buildString {
            append("-y ")
            ...
            ReturnCode.isCancel(code) ->
                ...
                if (coverImage != null) {
                    ...
                    if (ReturnCode.isSuccess(retry.returnCode) &&
                        outputMp3.exists() && outputMp3.length() > 0L
                    ) {
                        Result.success(outputMp3)
                    } else {
                        val fail = retry.failStackTrace?.takeIf { it.isNotBlank() }
                            ?: retry.output?.takeLast(400)
                            ?: "FFmpeg failed (code=${retry.returnCode?.value})"
                        Result.failure(IllegalStateException(fail))
                    }
                } else {
                    val fail = session.failStackTrace?.takeIf { it.isNotBlank() }
