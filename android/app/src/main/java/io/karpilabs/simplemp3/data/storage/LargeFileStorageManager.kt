...
class LargeFileStorageManager
    ...
    constructor(
        ...
        private fun coldDir(): File = File(context.filesDir, "storage/cold").also { it.mkdirs() }
        ...
        fun isLarge(track: TrackEntity): Boolean = track.size >= LARGE_MIN_BYTES || track.duration >= LARGE_MIN_DURATION_MS
        ...
        suspend fun ensurePlayable(track: TrackEntity): TrackEntity =
            mutex.withLock {
         ...
         */
        suspend fun optimizeIfNeeded(track: TrackEntity): TrackEntity =
            mutex.withLock {
        ...
        suspend fun runMaintenance(excludeTrackIds: Set<Long> = emptySet()) {
            mutex.withLock {
                if (appPreferences.isLargeFileOptimize()) {
                    val candidates =
                        trackDao.getUnoptimizedLargeTracks(
                ...
                if (appPreferences.isLargeFileColdPack()) {
                    ...
                    val idle =
                        trackDao.getHotIdleLargeTracks(
        ...
        suspend fun maybeColdPackAfterPlay(
            trackId: Long,
            activeIds: Set<Long>,
        ) {
        ...
        private suspend fun reencodeLean(track: TrackEntity): TrackEntity {
            ...
            val result =
                AudioConverter.convertToMp3AtBitrate(
                    input = input,
                    outputMp3 = out,
                    metadata =
